...
// NuttX has no /proc/mounts (or equivalent) and no mntent.h -- nothing
// in the current applet scope calls these, so this is a real gap left
// unimplemented rather than shimmed, same as toybox's own precedent for
// xattr on BSD below. A future mount-table applet needs a real design
// here (NuttX exposes mount info via getmntinfo()-shaped statfs arrays,
// not a text table), not a stub.
int mountlist_istype(struct mtab_list *ml, char *typelist)
{
  error_exit("no mount table support on NuttX");
}
...
// No xattr for bsd
#elif defined(__NuttX__)
#warning no xattr support on NuttX
// Same treatment as BSD just above: xattr_get() and friends are simply
// not declared (see the __APPLE__||__linux__-only prototypes in
// portability.h) or defined here. NuttX has no extended-attribute
// syscalls to wrap, so there's nothing to shim to -- consistent with
// how toybox's own authors already treat this exact gap on BSD.
#else
#error
#endif
...
int dev_minor(int dev)
{
  ...
  return minor(dev);
#elif defined(__NuttX__)
  // NuttX's include/sys/sysmacros.h defines real major()/minor()/
  // makedev() macros -- same BSD-style convention, not a stub.
  // Confirmed by a real build this needs the explicit include: unlike
  // BSD (where <sys/types.h>/<sys/mkdev.h> pull these in transitively
  // by convention), NuttX doesn't, and without it these three calls
  // silently became undeclared-function calls instead of macro
  // expansions -- compiles with only a warning, but leaves a call to
  // a nonexistent `minor`/`major`/`makedev` symbol for the linker to
  // fail on.
  return minor(dev);
#else
#error
...
char *fs_type_name(struct statfs *statfs)
{
  ...
  // Confirmed directly: NuttX's include/sys/statfs.h defines this exact
  // set of *_MAGIC/*_SUPER_MAGIC constants with the same values as
  // Linux (ext2/3/4, tmpfs, proc, romfs, nfs, ... all match) -- genuine
  // shared convention, not a coincidence, so the Linux table below is
...
#ifdef __NuttX__
      // Unreachable here: check_copy_file_range() below only returns
      // nonzero on Android or where __NR_copy_file_range is defined,
      // neither true on NuttX, so try_cfr is always 0 and this branch
      // never runs. Still has to compile, though -- and NuttX has no
      // raw syscall() facility at all (confirmed: no declaration, no
      // implementation anywhere in its headers or libc, unlike
      // Linux's syscall-table model), so calling it unconditionally
      // would leave the linker an unresolvable symbol for dead code.
      error_exit("copy_file_range unsupported on NuttX");
#else
