  ...
  int protocol, int flags)
{
  ...
  // getaddrinfo()/gai_strerror() are real NuttX functions (declared in
  // netdb.h) but only compiled in when CONFIG_LIBC_NETDB is set, which
  // itself depends on the network stack being enabled (NET_IPv4 ||
  // NET_IPv6) -- confirmed directly in libs/libc/netdb/Kconfig. Not
  // used by anything in the current applet scope, and forcing a full
  // network stack config on just to satisfy this one unreachable
  // codepath would be real overreach for a minimal true/false/echo/
  // pwd build. Genuine gap under this config, left honest.
  error_exit("networking unsupported in this NuttX config");
#else
