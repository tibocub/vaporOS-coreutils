...
// Unlike glibc, NuttX doesn't pull struct statfs in transitively via
// <sys/mount.h> -- needs the explicit include like the __linux__ case.
// No sys/swap.h or sys/sysinfo.h equivalent; nothing in the current
// applet scope needs them.
#include <sys/statfs.h>
#endif
...
#include <pty.h>
#ifndef IUTF8
// Linux gets this from <bits/termios.h> via <pty.h>. NuttX's termios.h
// doesn't define it at all (not just NuttX-specific -- any non-Linux,
// non-BSD, non-Apple target hitting this branch would have the same
// gap). Same fallback the __IsBSD__ branch below already uses.
#define IUTF8 0
#endif
#else
#include <termios.h>
...
#elif defined(__NuttX__)
// NuttX's struct statfs (include/sys/statfs.h) has no f_frsize field at
// all, unlike Linux -- same pre-f_frsize Unix convention as the
// Apple/BSD case above, just with f_bsize instead of f_iosize since
// NuttX doesn't have that field either.
...
#include <utmpx.h>
#else
// NuttX has no utmpx.h at all (confirmed directly, no header anywhere
// in its tree) -- __has_include alone isn't safe here: without an
// explicit __NuttX__ check it can find the *host's* utmpx.h instead
// (reachable since apps/external builds don't use -nostdinc), which
// assumes glibc's own __pid_t chain and fails to parse against
// NuttX's pid_t. shadow.h and sys/random.h just below don't have this
// problem -- NuttX provides its own real versions of both (confirmed
// directly), found correctly ahead of any host fallback since NuttX's
// own -isystem path is searched first.
...
#ifndef SYSLOG_NAMES
// NuttX's own <nuttx/compiler.h> (pulled in transitively by nearly
// everything NuttX-side, including dirent.h/signal.h/pthread.h/
// stdlib.h/poll.h/nuttx/fs/fs.h -- confirmed by a real build, not
// guessed) #defines CODE as an empty far-pointer/memory-model
// annotation macro, the same way FAR/NEAR are used. That collides
// with toybox's own `CODE` typedef name below.
//
// IMPORTANT: a plain #undef here is NOT safe -- toybox's own toys.h
// includes plenty of NuttX headers *after* this point in the same
// translation unit (dirent.h, signal.h, pthread.h, stdlib.h, sys/
// poll.h, nuttx/fs/fs.h and more), and all of them use CODE too.
// Permanently undefining it breaks every one of those downstream
// (confirmed directly: a real build with a bare #undef here failed
// with dozens of "two or more data types in declaration specifiers"
// errors, all in NuttX's own headers, not toybox's). push_macro/
// pop_macro save and restore the real definition around just these
// two lines instead.
#ifdef __NuttX__
