...
// NuttX has no way to swap out the whole environ array: only
// get_environ_ptr() (a getter) is exposed, confirmed directly in
// sched/environ/ -- there's no setter a real "environ = new_array"
// assignment could target. The generic xclearenv()/xsetenv() above
// assume they can malloc() a private array and repoint environ at it
// (a real glibc/musl/BSD idiom, per this file's own opening comment
// about not trusting libc not to leak) -- structurally impossible
// here, not just a syntax mismatch.
//
// NuttX's own setenv()/unsetenv()/clearenv() (sched/environ/) already
// manage the per-task-group environment's memory correctly, which is
// specifically what this file's leak-tracking (toys.envc, the
// free()-on-drop logic in the generic xsetenv() above) exists to work
// around on libcs that don't -- so delegating outright is the honest
// fix here, not a workaround for a gap. One real behavior difference:
// xpop_env()'s contract elsewhere is "hand back the live pointer
// instead of freeing it" (the caller takes ownership); NuttX owns its
// own environment storage, so this returns a copy instead. Every
// actual toybox caller only reads the returned string, so this is
// safe, but it is not literally the same pointer semantics.

void xclearenv(void)
{
  if (clearenv()) perror_exit("clearenv");
}

char *xsetenv(char *name, char *val)
{
  char *eq = strchr(name, '='), *key = name;
  int len = eq ? eq-name : strlen(name);
  char *ret;

  if (eq) {
    if (val) error_exit("xsetenv %s to %s", name, val);
    val = eq+1;
    key = xstrndup(name, len);
  } else if (!val) {
    if (unsetenv(name)) perror_exit("unsetenv %s", name);

    return 0;
  }

  if (setenv(key, val, 1)) perror_exit("setenv %s", key);
  ret = getenv(key);
  if (key != name) free(key);
...
char *xpop_env(char *name)
{
  ...
  for (len = 0; name[len] && name[len]!='='; len++);
  if (!(val = getenv(name))) return 0;
  s = xmprintf("%.*s=%s", len, name, val);
  if (unsetenv(name)) perror_exit("unsetenv %s", name);
