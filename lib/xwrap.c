...
{
#ifdef __NuttX__
  // NuttX has no chroot() at all (no declaration, no implementation
  // anywhere in its headers/libc, confirmed directly) -- it doesn't
  // have the traditional multi-process filesystem-namespace model
  // chroot() assumes. Not used by anything in the current applet
  // scope; a real gap left honest rather than faked, same treatment
  // as the mount-table/file-watch gaps above.
  error_exit("chroot unsupported on NuttX");
#else
