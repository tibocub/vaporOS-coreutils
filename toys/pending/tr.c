...
// NOTE(vaporOS): every TT.map[] index below is cast to unsigned char. Upstream
// indexes with plain (signed) char, so bytes >= 0x80 (tr -c always produces
// them, and so does any UTF-8 input) write/read TT.map[-128..-1], i.e.
// outside the array. Harmless-looking on glibc, heap corruption on NuttX.
static void map_translation(char *set1 , char *set2)
{
  int i = TT.len1, k = 0;

  if (FLAG(d))
    for (; i; i--, k++) TT.map[(unsigned char)set1[k]] = (unsigned char)set1[k]|0x100; //set delete bit

  if (FLAG(s)) {
    for (i = TT.len1, k = 0; i; i--, k++)
      TT.map[(unsigned char)set1[k]] = TT.map[(unsigned char)set1[k]]|0x200;
    for (i = TT.len2, k = 0; i; i--, k++)
      TT.map[(unsigned char)set2[k]] = TT.map[(unsigned char)set2[k]]|0x200;
  }
  i = k = 0;
  while (!FLAG(d) && set2 && TT.len1--) { //ignore set2 if -d present
    TT.map[(unsigned char)set1[i]] = ((TT.map[(unsigned char)set1[i]] & 0xFF00) | (unsigned char)set2[k]);
    if (set2[k + 1]) k++;
    i++;
  }
}
...
static void print_map(char *set1, char *set2)
{
  ...
  while ((n = read(0, toybuf, sizeof(toybuf)))) {
    if (n<0) perror_exit("read"); // upstream spins forever on a read error
    if (!FLAG(d) && !FLAG(s))
      for (dst = 0; dst < n; dst++) toybuf[dst] = TT.map[(unsigned char)toybuf[dst]];
    else for (src = dst = 0; src < n; src++) {
      ch = TT.map[(unsigned char)toybuf[src]];
