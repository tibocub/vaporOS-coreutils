...
#ifdef __NuttX__
// NuttX's strftime() has no %Z: inside a longer format it silently prints
// nothing, and a bare "%Z" makes it return 0 (reported as "bad format").
// Substitute the zone abbreviation ourselves before calling it.
static size_t nx_strftime(char *out, size_t max, char *fmt, struct tm *tm)
{
  const char *zone = tm->tm_zone;
  char *f, *d;
  size_t n;

  if (!zone || !*zone) zone = tzname[tm->tm_isdst>0];
  if (!zone || !*zone) zone = "UTC";
  d = f = xmalloc(strlen(fmt)*strlen(zone)+1);
  while (*fmt) {
    if (*fmt == '%' && fmt[1] == 'Z') d = stpcpy(d, zone), fmt += 2;
    else if (*fmt == '%' && fmt[1]) *d++ = *fmt++, *d++ = *fmt++;
    else *d++ = *fmt++;
  }
  *d = 0;
  n = strftime(out, max, f, tm);
  free(f);
