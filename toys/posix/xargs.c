...
#ifdef __NuttX__
    {
      // No fork(): spawn directly. Same as the child code below: stdin comes
      // from /dev/null (or /dev/tty with -o) unless -a was used, and the
      // slot number goes into the environment.
      struct vapor_spawn_opts so = {-1, -1, 0, 0, 0, 0};

      if (!TT.a) so.in_path = FLAG(o) ? "/dev/tty" : "/dev/null";
      if (TT.process_slot_var)
        so.setenv_kv = xmprintf("%s=%d", TT.process_slot_var, slot);
      if ((pid = vapor_spawn(out, &so)) == -1) {
        // What the failed child would have done: report, exit 126/127.
        toys.exitval = 126+(errno == ENOENT);
        perror_msg("exec %s", *out);
        pid = -1;
        TT.np--;
        free(so.setenv_kv);
        continue;
