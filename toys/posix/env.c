...
#ifdef __NuttX__
      {
        // No exec() on NuttX: run it as a child, exit with its status. Its
        // own PATH lookup (posix_spawnp) and the tbx fallback replace ours.
        struct vapor_spawn_opts so = {-1, -1, 0, 0, 0, 0, ee, TT.e};
        pid_t pid = vapor_spawn(ev, &so);

        if (pid != -1) _exit(xwaitpid(pid));
      }
#else
