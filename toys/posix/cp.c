...
#ifdef __NuttX__
  // NuttX has no xattr support at all (confirmed directly -- see
  // lib/portability.c's own honest-gap treatment: xattr_get/set/list
  // are left undeclared for NuttX, matching upstream's own precedent
  // for BSD). Nothing to copy. Warn once rather than silently
  // pretending it worked, but don't abort the whole cp over a
  // preserve mode this specific platform can't honor -- only reached
  // when --preserve=xattr or --preserve=all is explicitly requested,
  // not by plain -p/-a (confirmed: those only set _CP_mode|
  // _CP_ownership|_CP_timestamps).
  error_msg("%s: xattr/context preservation unsupported on NuttX", file);
#else
  ssize_t listlen, len;
